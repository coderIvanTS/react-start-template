...
const getDataAmountTypeGuard = (type: never): never => {
    throw new Error(`Function getDataAmount can be called with a new type of object that it knows nothing about`);
}
...
const getDataAmount = (data: Data): number => {
    switch (data.type) {
        ...
        case 'Percent':
            throw new Error(`Function getDataAmount impossible use with data type ${data.type}`);
        default: {
            getDataAmountTypeGuard(data.type); // здесь, возможно, нужно использовать нечто другое. :never должен остаться
        }
    }
};
